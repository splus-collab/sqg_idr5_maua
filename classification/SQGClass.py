...
class SQGClass:
        ...
        def __init__(self, model='auto', verbose=False, release = "DR5", request = False):
                self.model = model
                ...
                if request:
                        if self.release == "DR4":
                                self.model_splus = requests.get("https://splus.cloud/files/models/iDR4_RF_12S4M.sav")
                                self.model_wise = requests.get("https://splus.cloud/files/models/iDR4_RF_12S2W4M.sav")
                        elif self.release == "DR3" or self.release == "DR2":
                                ...
                                # self.model_splus = requests.get("INSERT")
                                # self.model_wise = requests.get("INSERT")
                        else:
                        ...
                        if self.release == "DR4":
                                with open("iDR4_RF_12S4M.sav", "rb") as f:
                                        self.model_splus = pickle.load(f)
                                with open("iDR4_RF_12S2W4M.sav", "rb") as f:
                                        self.model_wise = pickle.load(f)
                        elif self.release == "DR5":
                                with open("iDR5_RF_12S4M.sav", "rb") as f:
                                        self.model_splus = pickle.load(f)
        ...
        def classify(self, df, return_prob=True,match_irsa=False, columns_wise={'w1mpro': 'w1mpro',
                ...
                if self.model == "RF16":
                        ...
                        data_nowise.dropna(subset=self._morph+self._feat, inplace=True) #I srsly dont know why I had to drop NaN again! 
